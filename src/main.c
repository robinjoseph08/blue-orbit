...
static const int TIME_HEIGHT = 36;
...
static GPoint calculate_small_circle_position(tm *tick_time) {
  GPoint pos;
  int angle = TRIG_MAX_ANGLE * tick_time->tm_min / 60;
...
// ==========================
static void move_small_circle(struct tm *tick_time) {
...
static void update_minute(struct tm *tick_time) {
  ...
  // =======================
  // create a long-lived buffer
  static char date_buffer[] = "Jan 31";
  // write the current month and day into the buffer
  strftime(date_buffer, sizeof("Jan 31"), "%b %d", tick_time);\
  // set date on layer
  text_layer_set_text(s_date_layer, date_buffer);
  text_layer_set_text(s_date_shadow_layer, date_buffer);

  // update small circle
  move_small_circle(tick_time);
}
...
static void main_window_load(Window *window) {
  ...
  // == fonts ==
  // ===========
  s_time_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_MONTSERRAT_34));
  s_date_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_MONTSERRAT_16));

  // ================
  // == time layer ==
  // ================
  s_time_layer = text_layer_create(GRect(0, HEIGHT / 2 - (TIME_HEIGHT + DATE_HEIGHT) / 2 - TEXT_OFFSET, WIDTH, TIME_HEIGHT));
  text_layer_set_background_color(s_time_layer, GColorClear);
  text_layer_set_text_color(s_time_layer, GColorWhite);
  // time layer shadow
  s_time_shadow_layer = text_layer_create(GRect(0, HEIGHT / 2 - (TIME_HEIGHT + DATE_HEIGHT) / 2 - TEXT_OFFSET + SHADOW_OFFSET + 1, WIDTH, TIME_HEIGHT));
  ...
  // == date layer ==
  // ================
  s_date_layer = text_layer_create(GRect(0, HEIGHT / 2 + (TIME_HEIGHT - DATE_HEIGHT) / 2 - TEXT_OFFSET, WIDTH, DATE_HEIGHT));
  text_layer_set_background_color(s_date_layer, GColorClear);
  text_layer_set_text_color(s_date_layer, GColorWhite);
  // date layer shadow
  s_date_shadow_layer = text_layer_create(GRect(0, HEIGHT / 2 + (TIME_HEIGHT - DATE_HEIGHT) / 2 - TEXT_OFFSET + SHADOW_OFFSET + 1, WIDTH, DATE_HEIGHT));
  text_layer_set_background_color(s_date_shadow_layer, GColorClear);
  text_layer_set_text_color(s_date_shadow_layer, SHADOW_COLOR);
  // font
